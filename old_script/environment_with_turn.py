# 本脚本实现了使用滑块控制六足机器人在平整地面前进和转向的功能
...
class Hexapod(object):
    def __init__(self) -> None:
        ...
        self.forward_debugId = p.addUserDebugParameter("forward_length", 0.005, 0.1, 0.005, self.physicsClient)
    ...
    def gait(self):
        ...
        for i in range(NUM_LEGS):
            ...
            if self.cycle_leg_number_[i] == 0:
                ...
                swingLegTrajectory[0:HALF_CYCLE_LENGTH,1] = np.linspace(0, np.pi/3, HALF_CYCLE_LENGTH)
                swingLegTrajectory[HALF_CYCLE_LENGTH:CYCLE_LENGTH,1] = np.linspace(np.pi/3, 0, HALF_CYCLE_LENGTH)
                swingLegTrajectory[0:HALF_CYCLE_LENGTH,2] = np.linspace(0, -np.pi/2, HALF_CYCLE_LENGTH)
                swingLegTrajectory[HALF_CYCLE_LENGTH:CYCLE_LENGTH,2] = np.linspace(-np.pi/2, 0, HALF_CYCLE_LENGTH)
    ...
    def move_joints(self, trajectory):
        ...
        for i in range(CYCLE_LENGTH):
            ...
            p.setJointMotorControlArray(robot.hexapod, [0,1,2], p.POSITION_CONTROL, trajectory[0][i], forces=[60]*3)
            p.setJointMotorControlArray(robot.hexapod, [3,4,5], p.POSITION_CONTROL, trajectory[1][i], forces=[60]*3)
            p.setJointMotorControlArray(robot.hexapod, [6,7,8], p.POSITION_CONTROL, trajectory[2][i], forces=[60]*3)
            p.setJointMotorControlArray(robot.hexapod, [9,10,11], p.POSITION_CONTROL, trajectory[3][i], forces=[60]*3)
            p.setJointMotorControlArray(robot.hexapod, [12,13,14], p.POSITION_CONTROL, trajectory[4][i], forces=[60]*3)
            p.setJointMotorControlArray(robot.hexapod, [15,16,17], p.POSITION_CONTROL, trajectory[5][i], forces=[60]*3)
            p.stepSimulation()
...
if __name__ == "__main__":
    ...
    while True:
        forward_length = p.readUserDebugParameter(robot.forward_debugId, robot.physicsClient) / 1.4
        turn_radius = p.readUserDebugParameter(robot.turn_debugId, robot.physicsClient) * 1.4
